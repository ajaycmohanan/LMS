<?php
    ...
    $sql = 'SELECT * FROM librarian WHERE adminid = :userid';
    ...
    if (isset($_POST["updateinfo"]) && !empty($_POST)) {
        	...
        	if ($fileError === 0) {
        		if ($fileSize < 1000000) {
        			...
        			if(move_uploaded_file($fileTmpName, $fileDestination)) {
        				$sql = 'UPDATE librarian SET img = :img WHERE adminid = :adminid';
        				$stmt = $pdo -> prepare($sql);
        				$stmt -> execute(['img' => 1, 'adminid' => $userid]);
        			}
        		} else {
        ...
        $sql = 'UPDATE librarian SET name = :name, phno = :phno, address = :address, city = :city, state = :state, pincode = :pincode WHERE adminid = :adminid';
        $stmt = $pdo -> prepare($sql);
        $stmt -> execute($data);
        header("location: http://localhost/admin-master/profile.php");
    }
