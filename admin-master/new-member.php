<?php
    ...
    function mailVerify($email,$userid,$hash) {
      ...
      $mail->Body    = '
        ...
        Your account has been created with the Registration Id '.$userid.'. Please activate your account by pressing the url below.
          ...
          http://pusthakangal.com/verifymember.php?email='.$email.'&hash='.$hash.'&userid='.$userid.'
    ...
    if (isset($_POST["membership"]) && !empty($_POST)) {
        ...
        if($count > 0) {
        	...
        	$sql = 'INSERT INTO members (mid, adminid, name, email, phno, hname, place, city, state, pincode, hash, dom) VALUES (:mid, :adminid, :name, :email, :phno, :hname, :place, :city, :state, :pincode, :hash, :dom);';
        	$stmt = $pdo -> prepare($sql);
        	$stmt -> execute($data);
        	$count = $stmt -> rowCount();
        	if($count > 0)  {
		...
		<div class="pd-ltr-20 customscroll customscroll-10-p height-100-p xs-pd-20-10">
			<div class="min-height-200px">
				...
				<div class="pd-20 bg-white border-radius-4 box-shadow mb-30">
					<div class="clearfix">
						<div class="pull-left">
							<h4 class="text-blue">New Membership</h4><br>
						</div>
						<div>
							<?php
